...
const ProjectList: React.FC<ProjectListProps> = ({ filterServiceIds }) => {
  ...
  const loadProjects = async () => {
    try {
      ...
      const res = await fetch(`${API_URL}/users/public-project?${params}`);
      const data = await res.json();
  ...
  useEffect(() => { loadLeadIds(); }, []);
  useEffect(() => { loadProjects(); }, [search, page]);

  const filteredProjects = projects.filter((project: ProjectItem) =>
    filterServiceIds.length === 0 ||
    project.service_ids?.some((id: number) => filterServiceIds.includes(id))
  );
        ...
        {filteredProjects
          ...
          .map(item => (
            <ProjectCard
              ...
              title={item.title}
              user={item.user?.name || "Unknown User"}
              services={item.service_list?.map((s: any) => s.name).join(", ") || ""}
              luxury={item.luxury_level_details?.name || "N/A"}
              landSize={item.land_size}
              location={item.location || "N/A"}
              description={item.notes || ""}
              ...
              basement={item.basement || "N/A"}
              typeName={item.category?.name || "N/A"}
              serviceNames={item.service_list?.map((s: any) => s.name) || []}
              email={item.user?.email}
              phone={item.user?.phone || ""}
              profileImage={item.user?.profile_image || ""}
            />
